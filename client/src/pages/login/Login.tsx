...
export default function Login() {
  ...
  const { execute: loginUser, isLoading: loadingUser, error: errorUser } = useFetch('POST', '/auth/login-user');
  const { execute: loginAdmin, isLoading: loadingAdmin, error: errorAdmin } = useFetch('POST', '/auth/login');
  ...
  const handleUserLogin = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      login({ userId: res.data.user.id, email: res.data.user.email, role: 'user' });
      navigate('/');
    }
  };
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      login({ userId: res.data.admin.id, email: res.data.admin.email, role: 'admin' });
      navigate('/dashboard');
    }
  };
        ...
        <Tabs defaultValue="user" className="w-full">
          ...
          <TabsContent value="admin">
            <form onSubmit={handleAdminLogin} className="space-y-8">
              ...
              {errorAdmin && <p className="text-red-500 text-sm">{errorAdmin}</p>}
